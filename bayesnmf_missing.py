...
from nmf_missing import _safe_dot, _special_dot_X
__all__ = ['Bd']
...
class Bd(nmf_std.Nmf_std):
    ...
    **Stopping criterion**
    ...
    def __init__(self, V, seed=None, W=None, H=None, rank=30, max_iter=30,
        ...
        #print('V is', np.nansum(self.V))
        V_mask = np.isnan(self.V)
        
        if np.any(V_mask):
            self.V = np.ma.masked_array(self.V, mask=V_mask)
            self.V_mask = V_mask
        else:
            self.V_mask = False
    ...
    def factorize(self):
        ...
        self.v = np.nansum(np.multiply(np.mat(self.V), np.mat(self.V))) /2.
    ...
    def update(self, iter):
        ...
        for _ in range(self.skip * (iter == 0) + self.stride * (iter > 0)):
            # update basis matrix
            if self.V_mask is False:
                
                C = dot(self.H, self.H.T)
                D = dot(self.V, self.H.T)
                #print('C D shape', C.shape, D.shape)
            else:
                C = _safe_dot(self.H, self.H.T) 
                D = _safe_dot(self.V, self.H.T)
                #print('C D shape', C.shape, D.shape)
                #_special_dot_X(self.H, self.H.T, self.V)#, out = C) #_safe_dot(self.H, self.H.T)
#             V_mask = np.isnan(self.V)
#             if np.any(V_mask):
#                 V = np.ma.masked_array(self.V, mask=V_mask)
            #WH = _special_dot_V(self.W, self.H, V, out=WH)
            ...
            for n in range(self.rank):
                if not self.n_w[n]:
                    ...
                    #print('Value is ', (sop(D[:, n] - self.W[:, nn] * C[nn, n], C[
                    #        n, n] + np.finfo(C.dtype).eps, div)).shape)
                    #print('D shape', D[:, n].shape)
                    #print('W shape', _safe_dot(self.W[:, nn], C[nn, n]).shape)
                    num = D[:, n][:,np.newaxis] - _safe_dot(self.W[:, nn], C[nn, n])#
                    den = C[n, n] + np.finfo(C.dtype).eps
                    #print('my shape', self.W[:, nn].shape, C[nn,n].shape, D[:,n].shape, dot(self.W[:, nn], C[nn, n]).shape)
                    #print('Ratio', (num/den).shape)
                    #, self.W[:,nn].shape)#(num/den).shape)
                    temp = self._randr(
                        num/den,
            ...
            E = _safe_dot(self.W.T, self.W) #_special_dot_X(self.W.T, self.W, self.V, out = C) #dot(self.W.T, self.W)
            F = _safe_dot(self.W.T, self.V)
            for n in range(self.rank):
                if not self.n_h[n]:
                    nn = list(range(n)) + list(range(n + 1, self.rank))
                    temp = self._randr(
...
#         print('Ma', m[a])
    ...
    def objective(self):
        ...
        its NMF estimate."""
        #print(np.isnan(self.V).sum())
        val = np.nansum(power(self.V - dot(self.W, self.H), 2))
        print(val)
        return val
